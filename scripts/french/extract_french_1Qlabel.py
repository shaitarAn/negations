...
def write_files(data):
    ...
    outfile1 = outpath / Path('FRENCH_1label.json')
    textfile1 = outpath / Path('FRENCH_sents.txt')
    textfile2 = outpath / Path('FRENCH_1label_anno.txt')
...
for file in data:
    ...
    if 'ESSAI' in file:
        count_essai += len(sents)
        files.extend(['ESSAI'] * len(sents))

zipped_data = []
zipped_data3 = list(zip(scope_sents, scope_cues, data_scope, files))
zipped_data2 = [item for item in zipped_data3 if 0 not in item[1]]
